...
We've explored a few different ways to construct and/or fill ROIs with cells - both 2D and 3D, over the years. For the HuBMAP project, we were given .svg ROI geometries, whose boundaries were piecewise Bezier curves, and we ended up doing a scanline-style, hex-filled ROIs. And of course PhysiCell Studio (ICs tab) offers simple hex-filled geometries also. We also explored various [level set approaches](https://github.com/rheiland/kidney_level_sets) for some of the HuBMAP data.
